...
class Functor f where
  fmap :: (a -> b) -> (f a -> f b)
\end{haskell}
The function types involved in this definition, \hask{(a -> b)} and \hask{(f a -> f b)}, are the \emph{internal} homs. A Haskell \hask{Functor} is indeed an enriched functor. 
...
\end{haskell}
with the following definitions of \hask{eval} and \hask{coeval}:
\begin{haskell}
eval :: (a -> b, a) -> b
eval (f, a) = f a
\end{haskell}
