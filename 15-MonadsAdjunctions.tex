...
\end{haskell}
Here, we interpret \hask{(a -> m)} as a regular function from \hask{a} to the underlying set of a monoid \hask{m}. The result is a monoid morphism from the free monoid generated by \hask{a} (that is a list of \hask{a}'s) to \hask{m}. This is just one direction of the adjunction:
\[ \Set (a, U m) \cong \Cat{Mon} (F a, m) \]

We get the counit as a monoid morphism \hask{[m]->m} by applying \hask{foldMap} to identity. The result is \hask{foldMap id } or, in terms of \hask{foldr}:
\begin{haskell}
