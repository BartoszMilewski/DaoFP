...
\end{haskell}
We can interpret it as taking a function from \hask{a} to the underlying set of a monoid \hask{m} and producing a monoid morphism from the free monoid generated by \hask{a} to \hask{m}. We get the counit $\varepsilon_M$ by applying it to the identity, \hask{foldMap id }, or 
\begin{haskell}
...
Let's unpack these equations for the case of the state monad transformer. Here, the left functor is the product functor \hask{(a, s)} and the right functor is the exponential, a.k.a., the reader functor \hask{s->a}. The counit $\varepsilon^o_Y$ is function application, for the function \hask{f :: x -> y}:
\begin{haskell}
epsilon (f, x) = f x
\end{haskell}
...
Now, what is $R(\eta^i_{L a})$? It's the component of \hask{return} of the inner monad taken at $L a$. Here, $L a$ is the type \hask{(a, s)}. So we are instantiating \hask{return} as a function \hask{(a, s) -> m (a, s)}. (The type inferencer will do this automatically for us.)
