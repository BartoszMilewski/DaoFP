...
When an arrow $f$ is post-composed with an arrow $h$, it produces the arrow $f \circ h$. Of course, $f$ can be post-composed only with arrows whose target is the source of $f$. Post-composition by $f$ is written as $(f \circ -)$, leaving a hole for $h$. As Lao Tzu would say, ``Usefulness of post-composition comes from what is not there.''
...
Pre-composition let us shift the perspective from one observer to another. Notice that the outgoing arrows are mapped in the direction opposite to the arrow $f$.
