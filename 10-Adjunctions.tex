...
A free monoid generated by some type \hask{a} treated as a set of values, is represented by a list type \hask{[a]}. An empty list serves as the unit, and monoid multiplication is implemented as list concatenation, traditionally written in infix form:
\begin{haskell}
(++) :: [a] -> [a] -> [a]
(++) []     ys = ys
(++) (x:xs) ys = x : xs ++ ys
