...
We can now use the \index{Yoneda trick}Yoneda trick and pick the identity arrow $id_{L x}$ on the left. The isomorphism maps it to a unique arrow on the right, which we'll call $\eta_x$:
...
We can then use $\varepsilon$ to collapse the composite $L \circ R$ to identity. The result is an arrow $L x \to y$, which is an element of $ \mathcal{C} (L x, y)$.
...
\begin{exercise}
Given an arrow $g \colon L x \to y$ implement an arrow $x \to R y$ using $\eta$ and the fact that $R$ is a functor. Hint: Start with the object $x$ and see how you can get from there to $R y$ with one stopover.
\end{exercise}
...
\end{haskell}
The additional two clauses \hask{left -> right} and \hask{right -> left} tell the compiler that, when using an instance of the adjunction, one functor can be derived from the other. This definition requires the following compile extension:
\begin{haskell}
...
& \mathcal{D}(d, R c)
 \end{tikzcd}
\]
We can use the \index{Yoneda trick}Yoneda trick by setting $d'$ to $R c$.
...
It turns out that the set of cocones, which is an object in $\Set$, is itself a \emph{limit} of the following $\Set$-valued functor $F \colon \cat J^{op} \to \Set$:
\[ F j = \cat C(D j, x) \]
...
\begin{tikzcd}
 ...
 \arrow[rr, red]
 \arrow[dr, red]
 \arrow[dddr, bend right, ""']
&& D j_2
\arrow[dl, red]
 \arrow[dddl, bend left,  ""]
 \\
 & D j_3
 \arrow[dd, ""]
 \\
...
\begin{exercise}
Perform the defunctionalization of a binary tree traversal that returns the maximum of all leaves. Start with this CPS version:
\begin{haskell}
...
\begin{exercise}
Booleans can be equipped by two monoidal structures, AND and OR. Show that NOT defines a monoid isomorphisms between them. Hint: The two directions of this isomorphism are called \index{De Morgan's laws}De Morgan's laws.
\end{exercise}
