...
In category theory, though, not every endofunctor in a monoidal category is strong. For now, the magic incantation is that the category we're working with is self-enriched, and every endofunctor defined in Haskell is canonically enriched. We'll come back to it when we talk about enriched categories. In Haskell strength boils down to the fact that we can always \hask{fmap} a partially applied pair constructor \hask{(a, )}.
