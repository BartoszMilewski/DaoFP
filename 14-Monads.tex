...
Functors are structure-preserving maps between categories. If two objects are connected in the source category, their images are connected in the target category. They also preserve composition and identity, which means that they map commuting diagrams to commuting diagrams. What they do not preserve, in general, is universal constructions. 

For instance, this is the familiar diagram that defines a product:
\[
 \begin{tikzcd}
 & C
\arrow[d, dashed, "h"]
...
A && B
 \end{tikzcd}
\]
A functor $F$ will map this diagram to a similar diagram in the target category. There is no guarantee, though, that $F (A \times B)$ will be the product of $F A$ and $F B$ in that new category. Several things may go wrong to break the universality of $F (A \times B)$. The factorizing arrow $F h$ might not be unique, or there may be some extraneous objects with a pair of arrows to $F A$ and $F B$ for which there is no factorizing arrow.

With a generic tensor product, the problem is that the image of the functor might miss some objects in the target category.

If we want a functor $F$ from a monoidal category $\mathcal{C}$ to another monoidal category $\mathcal{D}$ to preserve the monoidal structure, we have to impose additional constraints. We may request that it maps tensor product to tensor product and unit to unit---up to isomorphism:
\[ F (A \otimes B) \cong F A \otimes F B \]
\[F I \cong I \]
where on the left we have the product and the unit in the source category, and on the right their counterparts in the target category.
