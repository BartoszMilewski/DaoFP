...
pairWith a = \x -> (a, x)
\end{haskell}
The function returned by calling \hask{pairWith 5} is such a closure.

Now consider this version, which returns a singleton list that contains our closure:
\begin{haskell}
pairWith' :: Int -> [String -> (Int, String)]
pairWith' a = [\x -> (a, x)]
\end{haskell}
As a programmer you'd be very surprised if this didn't work. But what we do here is highly nontrivial: we are smuggling the environment inside the list functor. According to our model of lambda calculus, a closure is a morphism from the product of the environment type and the function argument type, here \hask{(Int, String)}. 
...
\end{haskell}
The notation \hask{(a, )} is called a \index{tuple section}\emph{tuple section} and is equivalent to the partial application of the pair constructor: \hask{(,) a}.
...
that works nicely with the unitors and the associator of the monoidal category that defines the tensor product.
...
Even more importantly, every monad in Haskell is strong by virtue of being a functor. And this is also why every monad is automatically \hask{Monoidal}. 
\begin{haskell}
