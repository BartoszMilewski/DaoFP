...
In reality, because of Haskell's laziness, the execution of I/O is interleaved with the rest of the program.  Pure functions that comprise the bulk of your program are evaluated on demand---the demand being driven by the execution of the \hask{IO} script. If it weren't for I/O, nothing would be evaluated.
...
It's instructive to see how we could do it for the \hask{Maybe} functor. If the first function returns \hask{Just}, we pattern match it to extract the contents and call the next function with it.
...
Objects in this new category are the same Haskell types as before. But an arrow from \hask{a} to \hask{b} is implemented as a Haskell function:
\begin{haskell}
g :: a -> f b
\end{haskell}
...
m (m a) -> m (m a)
\end{haskell}
as an arrow in the Kleisli category that goes from \hask{ m(m a) } to \hask{m a}. Similarly, the function:
\begin{haskell}
m a -> m a
\end{haskell}
implements a Kleisli arrow from \hask{ m a} to \hask{a}. Their Kleisli composition produces a Kleisli arrow from \hask{ m (m a)} to \hask{a}, or a Haskell function:
\begin{haskell}
m (m a) -> m a
\end{haskell}
...
newtype Writer w a = Writer (a, w)
\end{haskell}
requires the type of the log to be an instance of \hask{Monoid}. This allows us to append logs and to create an empty log.
\begin{haskell}
...
fk :: a -> Cont r b
\end{haskell}
We'd like to run \hask{ma}, but for that we need a continuation that would accept an \hask{a}. We can build such a continuation: given an \hask{a} it would call \hask{fk}. The result is a \hask{b}-expecting continuation \hask{Cont r b}. But that's exactly the continuation \hask{k} we have at our disposal. Taken together, this convoluted process produces the following implementation:
\begin{haskell}
...
pairWith a = \x -> (a, x)
\end{haskell}
The function returned by calling \hask{pairWith 5} ``encloses'' the 5 from its environment.
...
instance Monad m => Monoidal m where
  ...
  ma >*< mb = do
    ...
    return (a, b)
\end{haskell}
(Warning: to compile this code you'll need to turn a few compiler extensions.) If you desugar this code to use bind and lambdas, you'll notice that the final \hask{return} needs access to both \hask{a} and \hask{b}, which are defined in outer environments. This would be impossible without a monad being strong.

In general, though, not every endofunctor in a monoidal category is strong. The reason it works in Haskell is that the category we're working with is cartesian closed. For now, the magic incantation is that such a category is self-enriched, so every endofunctor is canonically enriched. In Haskell this boils down to the fact that we can \hask{fmap} a partially applied pair constructor \hask{(a, )}.

\subsection{Applicative functors}

In programming, the idea of applicative functors arose from the following question: A functor lets us lift a function of one variable. How can we lift a function of two or more variables? By analogy with \hask{fmap}, we'd like to have a function:
\begin{haskell}
liftA2 :: (a -> b -> c) -> f a -> f b -> f c
\end{haskell}

A function of two arguments (here, in its curried form) is a function of one argument returning a function. So, assuming that \hask{f} is a functor, we can \hask{fmap} the first argument:
\begin{haskell}
a -> (b -> c)
\end{haskell}
...
Since, in a cartesian closed category, every monad is lax monoidal, it is automatically applicative. This can be shown directly by implementing \hask{ap}, which has the same type signature as the splat operator:
\begin{haskell}
ap :: (Monad m) => m (a -> b) -> m a -> m b
ap fs as = do 
    f <- fs
...
This connection is expressed in the Haskell definition of a \hask{Monad} that includes \hask{Applicative} as its superclass:
\begin{haskell}
class Applicative m => Monad m where
    ...
    return      = pure
\end{haskell}
Notice that the default implementation of \hask{return} as \hask{pure} is included. 
...
In programming, monad is more powerful than applicative. That's because monadic code lets you examine the contents of a monadic value and branch depending on it. This is true even for the \hask{IO} monad which otherwise provides no means of extracting the value, as in this example:
\begin{haskell}
main :: IO ()
main = do
  s <- getLine
