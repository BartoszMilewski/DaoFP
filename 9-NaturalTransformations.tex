...
\mathcal{C}
  \arrow[bend left=50, r, "F", ""{name=U, below}]
  \arrow[bend right=50, r, "G"', ""{name=D, above}]
&
\mathcal{D}
\arrow[Rightarrow, from=U, to=D, "\alpha", shorten <=5pt, shorten >=5pt]
\end{tikzcd}\]
...
\begin{tikzcd}[column sep=huge]
\mathcal{C}
  \arrow[bend left=60, rr, "F", ""{name=U, below}]
  \arrow[rr, "G"{xshift=-12pt}, ""{name=Mbot, below}, ""{name=Mtop, above}]
  \arrow[bend right=60, rr, "H"', ""{name=D, above}]
&&
\mathcal{D}
  \arrow[Rightarrow, from=U, to=Mbot, "\alpha", shorten >=5pt]
  \arrow[Rightarrow, from=Mtop, to=D, "\beta", shorten <=5pt]
\end{tikzcd}
\]

Let's look at the components of these transformations at some object $x$ in $\cat C$:
\[ \alpha_x \colon F \, x \to G \, x \]
...
\[ \gamma_x = \beta_x \circ \alpha_x \]
 This is called the \index{vertical composition}\emph{vertical composition} of natural transformations. You'll see it written using a dot $\gamma = \beta \cdot \alpha$ or a simple juxtaposition $\gamma = \beta \alpha$.
...
\end{tikzcd}
\]
The \index{horizontal composition}\emph{horizontal composition} $\beta \circ \alpha$ maps $G \circ F$ to $G' \circ F'$.
\[
\begin{tikzcd}[column sep=huge]
\mathcal{C}
...
We can translate this construction directly to Haskell. We start with two natural transformations:
\begin{haskell}
...
Because of the characteristic shape of the diagram, such composition is called \index{whiskering}\emph{whiskering}.
\[
\begin{tikzcd}[column sep=huge]
\mathcal{C}
 \arrow[r, "F"]
...
\end{tikzcd}
\]
The \index{interchange law}\emph{interchange law} states that the order of composition doesn't matter: we can first do vertical compositions and then the horizontal one, or first do the horizontal compositions and then the vertical one.
...
\[ F x = \cat C(a, x) \]
Such functors, and all the functors isomorphic to those, are called \index{representable functor}\emph{representable}. The whole functor is ``represented'' by a single object $a$. 
...
Seen this way, the representing object $a$ is like a \index{logarithm}logarithm of a functor. 
...
Both approaches have their advantages. The categorical approach came later, because you need to study a lot of examples before clear patterns emerge. But once you see the patterns, you can't unsee them. You discover unexpected connections between things, like the duality between sums and products.
...
But first let's study several examples in greater detail to get some understanding of the terse language of category theory. We'll try, for instance, to decode the statement that the sum, or the coproduct of two objects, can be tersely defined by the following natural isomorphism:
...
Even such a simple task as pointing at an object has a special interpretation in category theory. We have already seen that pointing at an element of a set $S$ is equivalent to selecting a function from the singleton set to $S$. Similarly, picking an object in a category is equivalent to selecting a functor from the single-object category. Alternatively, it can be done using a constant functor from another category. 
...
& c
 \end{tikzcd}
\]
This diagram can be further decomposed into two simpler shapes called \index{cospan}\emph{cospans}:
\[
 \begin{tikzcd}
 a
 \arrow[dr, ""']
...
Let's consider what happens when we start varying the object $x$ in a cospan. For every $D \colon \mathbf{2} \to \mathcal{C}$, we have a mapping $F_D \colon \cat C \to \Set$ that takes an object $x$ to the set of cospans over $x$:
\[ F_D x = [\mathbf{2}, \mathcal{C}](D, \Delta_x) \]
This mapping can be made functorial in $x$.
...
\[ \nu_1 \colon a \to y \]
We can get from $\mu_1$ to $\nu_1$ simply by post-composing it with $m \colon x \to y$. So the lifting of $m$ can be defined as a component-by-component post-compositon $(m \circ -)$:
\begin{align*}
\nu_1 = m \circ \mu_1 \\
\nu_2 = m \circ \mu_2 \\
\end{align*}
...
Notice also that the same equation can be interpreted as the statement that, for any diagram $D \colon \mathbf{2} \to \mathcal{C}$, the functor:
\[ F_D x = [\mathbf{2}, \mathcal{C}](D, \Delta_x) \]
is representable, and its representing object is $a + b$.

\subsection{Product as a universal span}

An analogous argument can be made for the universal construction for the product. Again, we start with the stick-figure category $\mathbf{2}$ and the functor $D$. But this time we use a natural transformation going in the opposite direction
\[ \alpha \colon \Delta_x \to D \]
Such a natural transformation is a pair of arrows that form a \index{span}\emph{span}:
\[
 \begin{tikzcd}
 &x
 \arrow[dl, "f"']
...
We can show that that the mapping:
\[ H_D x = [\mathbf{2}, \mathcal{C}](\Delta_x, D) \]
can be interpreted as a contravariant functor $H_D \colon \cat C^{op} \to \Set$. If this functor is representable, its representing object is $a \times b$:
\[ H_D x \cong \mathcal{C}(x, a \times b) \]


\begin{exercise}
Define the action of $H_D$ on morphisms in order to interpret it as a contravariant functor.
\end{exercise}
...
Unlike in the previous examples, here both hom-sets are in the same category, and it's easy to analyze the isomorphism in more detail. In particular, we'd like to see how the commuting triangle condition:
\[  f = \varepsilon_{a b} \circ (h \times id_a) \]
arises from naturality.

\subsection{Commuting condition from naturality}

The standard \index{Yoneda trick}Yoneda trick is to take the defining diagram and make a substitution for $x$ that would reduce one of the hom-sets to an endo-hom-set, that is a hom-set whose source is the same the target. This will allow us to pick a canonical element of that hom-set, that is the identity arrow.
...
apply = uncurry id
\end{haskell}
This my be surprising at first, until you realize that the currying of the function \hask{(a->b,a)->b} leads to \hask{(a->b)->(a->b)}.

We can also encode the two sides of the main isomorphism of hom-sets as Haskell functors:
\begin{haskell}
...
\end{haskell}
They are both contravariant functors in the type variable \hask{x}. First we have:
\begin{haskell}
...
Let's now apply the \index{Yoneda trick}Yoneda trick to it and replace $y$ with $b^a$. This also allows us to replace $g$ (which now goes for $x$ to $b^a$) with $h$. 
...
At the bottom, the pre-composition with $h$ acting on identity produces $h$ in the lower right corner. 

Finally, $\alpha^{-1}$ acting on $h$ produces $f$ in the upper right corner. 
...
\subsection{Exponential as a representing object}

We can interpret the mapping of an object $x$ to the set $\cat C(x \times a, b)$ as a contravariant functor $H_{a b} \colon \cat C^{op} \to \Set$. If this functor is representable, its representing object is the exponential $b^a$:

\[ H_{a b} x \cong \mathcal{C}(x, b^a)\]

\begin{exercise}
Define the action of $H_{a b}$ on morphism in order to interpret it as a contravariant functor.
\end{exercise}
...
As the source functor we'll still use the constant functor $\Delta_x \colon \cat J \to \mathcal{C}$.

A  natural transformation $\nu$, that is an element of the hom-set:
...
Since the indexing category may now contain arrows, the naturality conditions for these diagrams are no longer trivial. The constant functor $\Delta_x$ shrinks all vertices to one, so naturality squares shrink down to triangles. Naturality means that all triangles with $x$ in their apex must now commute. 
...
\[ [\cat J, \Set](\Delta_1, D)  \cong \Set(1, \text{Lim}D) \]
...
We can now say that the product is a limit (and the sum, a colimit) of a diagram from the indexing category $\mathbf{2}$.
...
\subsection{Limits and colimits as representing objects}

Cones over a diagram $D$ form a category. A morphism in the cone category is an arrow that connects two apexes (here $h \colon x \to y$), which makes all triangles commute (see, for instance, the blue triangle below).
\[
 \begin{tikzcd}
  x
 \arrow[rr, dashed, bend left, blue, "h"]
 \arrow[dd]
 \arrow[dddr, blue, "\nu_3"' near start]
\arrow[ddrr ]
 && y
 \arrow[ddll]
 \arrow[dddl, blue, "\mu_3" near start]
 \arrow[dd]
\\
\\
D 1 
...
& D 3
 \end{tikzcd}
 \]
 A limit is just a terminal cone in the category of cones over $D$.
 
 The mapping that assigns a set of cones to an apex defines a contravariant functor $H_D \colon \cat C^{op} \to \Set$:
 \[ H_D x = [\cat J, \mathcal{C}](\Delta_x, D) \]
An element of this set is a cone---a natural transformation $\nu$, whose components $\nu_i$ form the sides of the cone. 

Functoriality means that we can lift any morphism $h \colon x \to y$ to a function between sets of cones:
 \[ H_D h \colon H_D y \to H_D x \]
 In other words, given a cone $\mu \in [\cat J, \mathcal{C}](\Delta_y, D)$, we can construct a cone $\nu \in [\cat J, \mathcal{C}](\Delta_x, D)$. We define the sides of this cone, the components of the natural transformation $\nu$, by precomposition with $h$:
 \[ \nu_i = \mu_i \circ h \]
 \[
 \begin{tikzcd}
  x
...
\arrow[ddrr, "\nu_2"' near start]
 && y
 \arrow[ddll, blue, "\mu_1"' near start]
 \arrow[dd, "\mu_2"']
\\
\\
D 1 
...
The commuting conditions for the cone $\nu$ follow from the commuting conditions for the cone $\mu$. For instance, if we precompose the equation $f \circ \mu_1 = \mu_2$ with $h$, we get $f \circ \mu_1 \circ h = \mu_2 \circ h$, which gives us $f \circ \nu_1 = \nu_2$.

We can now define the limit as the representing object for our functor $H_D$:

\[ H_D x  \cong \mathcal{C}(x, \text{Lim}D) \]


Similarly, we can define a covariant functor:
\[ F_D x =  [\cat J, \mathcal{C}](D, \Delta_x)  \]
that assigns a set of cocones to an object $x$. The representing object for this functor is the colimit:
\[ F_D x  \cong \mathcal{C}( \text{Colim}D, x) \]
...
In category theory all these patterns are embodied in a single construction called an equalizer. An \index{equalizer}\emph{equalizer} is a limit of a diagram whose pattern is given by a stick-figure category with two parallel arrows:
\[
\begin{tikzcd}
...
A \index{coequalizer}\emph{coequalizer} is the colimit of the same diagram that we used to define the equalizer:
\[
\begin{tikzcd}
a 
\arrow[rr, red, shift left=.75ex, "f"]
...
Consider an example where $A$ is a set of pairs of integers \hask{(m, n)}, such that either both are even or both are odd.  We want to coequalize two functions that are the two projections \hask{(fst, snd)}. The coequalizer set $C$ will have two elements corresponding to two buckets. We'll represent it as \hask{Bool}. The coequalizing function \hask{q} selects the bucket:
\begin{haskell}
q :: Int -> Bool
q n = n `mod` 2 == 0 
\end{haskell}
Any function \hask{q'} that cannot distinguish between the components of our pairs, and is sensitive only to their parity:
\[ q' \circ \mathvar{fst} = q' \circ \mathvar{snd} \]
 can be uniquely factorized through the function \hask{h} (which depends on \hask{q'}):
\begin{haskell}
h :: (Int -> a) -> Bool -> a
h q' True  = q' 0
h q' False = q' 1
...
\begin{exercise}
Run a few tests that show that the factorization $(h\, q') \circ q$ gives the same result as \hask{q'}, where \hask{q'} given by the following definition:
\begin{haskell}
import Data.Bits
...
The functor $\mathcal{Y}$ is called the \index{Yoneda functor}\emph{Yoneda functor}. It's a mixed variance functor (a profunctor), When we fix its second argument it yields a presheaf $\mathcal{Y}_x \colon [\cat C^{op}, \Set]$. When we fix the first argument it produces a co-presheaf $\mathcal{Y}^x \colon [\cat C, \Set]$:
\[ \mathcal{Y}^x  = \cat C(x, -) \]
...
But that's not all: every arrow in $\mathcal{C}$ is mapped to a \emph{different} arrow. A functor that is injective on arrows is called \index{faithful functor}\emph{faithful}. 

If that weren't enough, the mapping of hom-sets is also \index{surjection}\emph{surjective}, meaning that every arrow between objects in $[\mathcal{C}^{op},  \mathbf{Set}]$ comes from some arrow in $\mathcal{C}$. A functor that is surjective on arrows is called \index{full functor}\emph{full}. 
