...
In a neatly organized universe, objects are always objects and arrows are always arrows. Except that sometimes sets of arrows can be thought of as objects. Slice categories go even further: they turn individual arrows into objects. 

A slice category describes how a particular object $c$ is seen from the perspective of its category $\cat C$. It's the totality of arrows pointing at $c$. But to specify an arrow we need to specify both its ends. One of these ends is $c$, so we only need to specify the other.
