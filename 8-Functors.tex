...
We can also strip a category of all arrows (except for the identity arrows). Such a bare-object category is called a \emph{disctrete} category or a set\footnote{Ignoring ``size'' issues.}. Since we associate arrows with structure, a set is a category with no structure. 
...
To first approximation, we can model programming in the category of sets. We often think of types as sets of values, and functions as set-theoretical functions. 

lots of concepts in category theory are derived from set theory

elements

assembly language of mathematics
