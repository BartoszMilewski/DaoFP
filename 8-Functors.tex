...
Based on the definition of a category, we can come up with all kinds of weird categories. For instance, there is a category that has no objects and no arrows. It satisfies all the condition of a category vacuously. There's another one that contains a single object and a single arrow (can you guess what arrow it is?). There's one with two unconnected objects, and one where the two objects are connected by a single arrow (plus two identity arrows), and so on. These are examples of what I call \index{stick-figure category}stick-figure categories\footnote{after Grothendieck, who calls them ``dessins d'enfants.''}---categories with a small handful of objects and arrows.
\subsection{Category of sets}

We can also strip a category of all its arrows (except for the identity arrows). Such a bare-object category is called a \index{discrete category}\emph{discrete} category or a set\footnote{Ignoring ``size'' issues.}. Since we associate arrows with structure, a set is a category with no structure. 
...
One such category is called the \index{opposite category}\emph{opposite} category. This is the category in which all the original arrows are inverted: what is called the source of an arrow in the original category is now called its target, and vice versa. 
...
An object in the  \index{slice category}\emph{slice category} $\mathcal{C}/c$ (also known as the \index{over category}over-category) is a pair $\langle e, p \rangle$, with $p \colon e \to c$.  
...
There is a dual notion of a \index{coslice category}coslice category $c / \mathcal{C}$, also known as an \index{under-category}under-category. It's a category of arrows emanating from a fixed object $c$. Objects in this category are pairs $\langle a, i \colon c \to a \rangle$. Morphisms in $c / \mathcal{C}$ are the arrows that make the relevant  triangles commute.
\[
 \begin{tikzcd}
& c
 \arrow[rd, "j"]
...
If you can think of a data type as a container of values, you should be able to apply a function to transform all these values, and create a transformed container. When this is possible, we say that the data type is \emph{functorial}. 
...
dimap f g h = g . h . f
\end{haskell}
You are applying the transformation to a function \hask{h :: a -> b} that has a ``receptor'' for the values of type \hask{a}, but you'd like to feed it the input of type \hask{a'}. This is only possible if you have a converter from \hask{a'} to \hask{a}, namely a function \hask{f :: a' -> a}.
...
A functor must also satisfy some laws: it must preserve composition and identity. These laws cannot be expressed in Haskell, but should be checked by the programmer. We have previously seen the definition of \hask{badMap} that didn't satisfy the identity laws, yet it would be accepted by the compiler. It would define an ``unlawful'' instance of \hask{Functor} for the list type constructor \hask{[]}.
...
We also have a constant functor $\Delta_c$ that maps all objects to a single object $c$, and all arrows to the identity arrow on this object. In Haskell, it's a family of functors parameterized by the target type \hask{c}:
\begin{haskell}
...
Functors from the opposite category $\mathcal{C}^{op}$ are called \index{contravariant functor}\emph{contravariant}. They have the property of lifting arrows that go in the opposite direction. Regular functors are sometimes called \index{covariant functor}\emph{covariant}.
...
Notice that parentheses are important here. A similar function \hask{a -> Bool -> Bool} has \hask{a} in a \emph{negative} position. That's because it's a function of \hask{a} returning a function \hask{(Bool -> Bool)}. Equivalently, you may uncurry it to get a function that takes a pair: \hask{(a, Bool) -> Bool}. Either way, \hask{a} ends up in a negative position.
