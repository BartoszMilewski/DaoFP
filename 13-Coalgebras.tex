...
Similarly, there is a unique coalgebra morphism from the initial algebra (which is also a coalgebra) to the terminal coalgebra. In fact, it can be shown that it's the same underlying morphism $\tau \colon \mu F \to \nu F$ in both cases. 

In the category of sets, the carrier set of the initial algebra is a subset of the carrier set of the terminal coalgebra, with the function $\tau$ embedding the former in the latter. 
\[
\begin{tikzpicture}
...
\end{haskell}
In this form, we can take it as the definition of the carrier for the terminal coalgebra. We can replace \hask{Fix f} with the type we are defining---let's call it \hask{Nu f}. The type signature:
\begin{haskell}
