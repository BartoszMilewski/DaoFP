...
In type theory we postulate that, given any pair of types $A$ and $B$, we can construct a new type $A \times B$. This defines the \emph{type constructor} for the product.

The introduction rule takes two terms, $x \colon A$ and $y \colon B$, and constructs the term $(x, y) \colon A \times B$. The notation $(,)$ corresponds to the \emph{data constructor} for the product.

There are two elimination rules. Given a term $z \colon A \times B$ we can extract a term of the type $A$ using $\text{fst}$, and  the term of type $B$ using $\text{snd}$\footnote{These projections are often called $\pi_1$ and $\pi_2$}.

The computation rule tells us that applying an eliminator to a term created using the introduction rule reproduces the corresponding argument. That is  $\text{fst} (x, y) = x$ and  $\text{snd} (x, y) = y$.
The computation rule can be illustrated in our categorical diagram by the two commuting triangles:
\[
 \begin{tikzcd}
 & 1
\arrow[d, dashed, "{(x, y)}"]
...
Finally, the uniqueness property tells us that introduction \emph{after} elimination reproduces the original. That is, given any $p \colon A \times B$, we have $(\text{fst}\,p, \text{snd}\, p ) = p$.

\subsection{Type theory patterns}

The type-theoretical description of products follows a very general pattern for constructing new types. It starts by defining a type constructor. We've seen type constructors that take zero or more types as arguments. Later we'll see that it's also possible to have type constructors that take values as arguments. These are called \emph{dependent types}.

Introduction and elimination rules tell us how to construct and eliminate terms of the new type. 
\[ \text{building blocks} \xrightarrow{intro} \text{term of type T} \]
\[ \text{term of type T} \xrightarrow{elim} \text{building blocks} \]

Computation rules tell us that elimination after introduction produces the same building blocks that were used in the construction. Every term of the type $T$ remembers its creation story.
\[ \text{building blocks} \xrightarrow{intro} \text{term of type T} \xrightarrow{elim} \text{building blocks} \]
In programming, the application of a computation rule is called \index{beta-reduction}\emph{beta-reduction}, or \index{$\beta$-reduction}$\beta$-reduction.


Optional uniqueness rules close the cycle by asserting that all terms of the new type are in fact the result of applying introduction rules to their constituents. 
\[ \text{term of type T} \xrightarrow{elim} \text{building blocks} \xrightarrow{intro} \text{term of type T} \]
In programming, the application of a uniqueness rule is called \index{eta-conversion}\emph{eta-conversion}  (\index{$eta$-conversion}$\eta$-conversion).
...
This is a perfectly valid definition. In practice, however, we are often interested in monoids that are embedded in larger categories. We call such monoids \index{internal monoid}\emph{internal}. In particular, in programming, we want to be able to define monoids inside the category of types and functions. 
...
We could translate this description directly to Haskell by defining a \index{type class}class of types equipped with two methods, traditionally called \hask{mappend} and \hask{mempty}:
\begin{haskell}
class Monoid m where
  mappend :: (m, m) -> m
  mempty  :: () -> m
\end{haskell}
We'll see the actual Haskell library definition shortly.
...
Thus if we replace $\times$ with $\otimes$ and $1$ with $I$, we get a definition of a monoid in an arbitrary monoidal category. We can \emph{internalize} a monoid inside a monoidal category.
...
Notwithstanding its simplicity, a monoid has several moving parts. It has the unit, the binary operation, and the three laws. 

If we strip it to just the binary operation with no unit and no associativity, we get the primordial structure called a \index{magma}\emph{magma}. A magma with associativity is called a \index{semigroup}\emph{semigroup}. Since in Haskell we have no way to express the laws, we cannot distinguish between a magma and a semigroup. So we define one class with a binary infix operator \hask{<>}:
\begin{haskell}
class Semigroup a where
    (<>) :: a -> a -> a
\end{haskell}

We can now define a monoid as a semigroup with unit (and unit laws). In Haskell, we express this dependency by requiring that \hask{Semigroup} be a \index{superclass}\emph{superclass} of \hask{Monoid}:
\begin{haskell}
class Semigroup a => Monoid a where
    mempty :: a
\end{haskell}
This way \hask{Monoid} inherits the binary operation \hask{<>} from \hask{Semigroup}, and adds to it the unit element \hask{mempty}.

An interesting example of a monoid is provided by endomorphisms, that is morphisms whose source and target are the same:
\begin{haskell}
newtype Endo a = Endo (a -> a)
\end{haskell}
The binary operation is given by composition:
\begin{haskell}
instance Semigroup (Endo a) where
  (Endo f) <> (Endo g) = Endo (f . g)
\end{haskell}
The unit of composition is the identity morphism, so we have:
\begin{haskell}
...
\begin{exercise}
Define the \hask{Monoid} instance for booleans using AND as the binary operation. To distinguish it from other monoids over \hask{Bool}, use the \hask{newtype} trick:
\begin{haskell}
...
\begin{exercise}
Define the \hask{Monoid} instance for booleans using OR as the binary operation. Use a different  \hask{newtype}:
\begin{haskell}
