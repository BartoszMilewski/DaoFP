...
The introduction rule takes two terms, $x \colon A$ and $y \colon B$, and constructs the term $(x, y) \colon A \times B$. This corresponds to the \emph{data constructor} for the product.
...
The computation rule tells us that applying an eliminator to a term created using the introduction rule reproduces the corresponding argument. That is  $\text{fst} (x, y) = x$ and  $\text{snd} (x, y) = y$. In programming we call this \index{beta reduction}\emph{beta reduction}.
...
Finally, the uniqueness property tells us that introduction \emph{after} elimination reproduces the original. That is, given any $p \colon A \times B$, we have $(\text{fst}\,p, \text{snd}\, p ) = p$. In programming we call this \index{eta reduction}\emph{eta reduction}.
