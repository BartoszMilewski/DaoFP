...
& ...
  \end{tikzcd}
\]
and plug $h$ and $\mathit{init}$ into it:
\[
 \begin{tikzcd}
 1
 \arrow[r, "Z"]
...
This single function, which is called a \emph{recursor}, is enough to implement all recursive functions on natural numbers. For instance, this is how we could implement addition:
...
plus n m = case m of
  ...
  (S k) -> S (plus n k)
\end{haskell}
It can be read as: If \hask{m} is zero then the result is \hask{n}. Otherwise, if \hask{m} is a successor of some \hask{k}, then the result is the successor of  \hask{n + k}. This is exactly the same as saying that \hask{init = n} and \hask{step = S}.
