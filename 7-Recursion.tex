...
Corresponding to the recursor, here's the type signature of the library function \hask{foldl} (fold \emph{left}) which is defined in the standard library:
\begin{haskell}
foldl :: (c -> a -> c) -> c -> [a] -> c
\end{haskell}
...
We can, for instance, use \hask{foldl} to calculate the sum of the elements of a list of natural numbers:
\begin{haskell}
sum :: [Nat] -> Nat
sum = foldl plus Z
\end{haskell}
...
The simplest choice would be $snd$, but it doesn't work, because it doesn't satisfy the commuting condition:
\[ \text{step} \circ (id_a \times h) = h \circ \text{Cons}_A \]
