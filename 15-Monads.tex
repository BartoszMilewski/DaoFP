...
In a cartesian \emph{closed} category, every monad\footnote{Again, the correct incantation is ``every enriched monad''}, being \hask{Monoidal}, is automatically \hask{Applicative}. We can show it directly by implementing \hask{ap}, which has the same type signature as the splat operator:
\begin{haskell}
...
What imperative languages do using nested loops we can do in Haskell using the list monad. Think of \hask{as} in bind as aggregating the result of running the inner loop and \hask{k} as the code that runs in the inner loop. 
...
Some Kleisli arrows, like \hask{putStrLn :: String -> IO ()}, produce no values---they are used only for effects. They can be bound to arrows that take no values, like \hask{getLine :: IO String} (equivalent to a Kleisli arrow \hask{() -> IO String}). Since this kind of binding is a common occurrence, the standard library defines a helper operator:
\begin{haskell}
(>>) :: Monad m => m a -> m b -> m b
ma >> mb = ma >>= \_ -> mb 
\end{haskell}
...
We would like a free monad to be generated by a left adjoint to this forgetful functor. The problem is that this left adjoint doesn't always exist. As usual, this is related to size issues: monads tend to blow things up. The bottom line is that free monads exist for some, but not all, endofunctors. Therefore we can't define a free monad through an adjunction. Fortunately, in most cases of interest, a free monad can be defined as a fixed point of a higher-order endofunctor. 
