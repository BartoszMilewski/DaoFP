...
The first observation is that, if we have an arrow \hask{h} from \hask{Bool} to some type \hask{A} then we automatically get two arrows $x$ and $y$ from unit to \hask{A}, just by composition. The following diagram commutes:
...
In general, a data type is created using \emph{introduction} rules and deconstructed using \emph{elimination} rules. The \hask{Bool} data type has two introduction rules, one using \hask{True} and another using \hask{False}. The \hask{if}, \hask{then}, \hask{else} construct defines the elimination rule. 

The fact that, given \hask{h}, we can reconstruct the two terms used to define it, is called the \emph{computation} rule. It tells us how to compute the result of \hask{h}. If we call \hask{h} with \hask{True}, then the result is \hask{x}; if we call it with \hask{False}, the result is \hask{y}.
...
Let's do a few examples. We haven't defined many types yet, so we'll be limited to mappings out from \hask{Bool} to either \hask{Void}, \hask{()}, or \hask{Bool}. 
...
\text{Bool}
 \end{tikzcd}
\]
How many pairs of functions from \hask{()} to \hask{Bool} do we have at our disposal? There are only two such functions, \hask{True} and \hask{False}, so we can form four pairs. These are $(True, True)$, $(False, False)$, $(True, False)$, and $(False, True)$. Therefore there can only be four functions from \hask{Bool} to \hask{Bool}. We can write them in Haskell using the  \hask{if}, \hask{then}, \hask{else} construct. For instance, the last one, which we'll call \hask{not} is defined as:
\begin{haskell}
not :: Bool -> Bool
...
c = Blue
\end{haskell}
To define mappings out of \hask{RGB}, we need a more general elimination pattern. Just like a function from \hask{Bool} was determined by two elements, a function from \hask{RGB} to \hask{A} is determined by a triple of elements of \hask{A}: \hask{x}, \hask{y}, and \hask{z}. We can write such function using \emph{pattern matching}:
\begin{haskell}
h :: RGB -> A
h Red   = x
h Green = y
h Blue  = z
\end{haskell}
This is just one function whose definition is split into three cases. It's possible to use the same syntax for \hask{Bool} as well, in place of \hask{if}, \hask{then}, \hask{else}:
\begin{haskell}
h :: Bool -> A
h True  = x
h False = y
...
These are not infinite types. Their elements can be enumerated, even if it takes ten thousand years. The type \hask{Integer} is infinite, though.
...
You can use \hask{undefined} when you're only interested in compiling, as opposed to running, your code. For instance, you may need to plug a function \hask{f} to check if your definitions work together:
\begin{haskell}
f :: a -> x
f = undefined
\end{haskell}
...
The \hask{Bool} type could be seen as the sum $2 = 1 + 1$. But nothing stops us from replacing $1$ with another type, or even replacing each of the $1$s with a different type. We can define a type $A + B$ using two arrows. Let's call them \hask{Left} and \hask{Right}. The defining diagram is the introduction rule:
\[
 \begin{tikzcd}
 A
 \arrow[dr,  "\text{Left}"']
...
You can also read these equations right to left, and you will see the computation rules for sum types. The two functions that were used to define \hask{h} can be recovered by applying \hask{h} to terms constructed using \hask{Left} and \hask{Right}. 
...
\hask{Maybe} is mostly used to encode the return type of partial functions: ones that are undefined for some values of their arguments. In that case, instead of failing, such functions return \hask{Nothing}. In other programming languages partial functions are often implemented using exceptions.
...
Since the sum is defined by the mapping out property, it was easy to see what happens when we change focus: it changes ``naturally'' with the foci of the arrows that define the product. But what happens when we move the sources of those arrows? Suppose that we have arrows that map $A$ and $B$ to some $A'$ and $B'$:
\[f \colon A \to A' \]
\[g \colon B \to B'\]
...
\[A + B \cong B + A \]
\[(A + B) + C \cong A + (B + C) \]
and it's functorial. A category with this type of operation is called \emph{symmetric monoidal}. When the operation is the sum (coproduct), it's called \emph{cocartesian}. In the next chapter we'll see another monoidal structure that's called called \emph{cartesian} without the ``co.''
