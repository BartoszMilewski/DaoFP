...
We have another ``buddy system,'' or a bijection, at work here. This time it's a two-way mapping between a pair of elements $(x, y)$ and an arrow $h$. 
...
In general, a data type is created using \emph{introduction} rules and deconstructed using \emph{elimination} rules. The \hask{Bool} data type has two introduction rules, one using \hask{True} and the other using \hask{False}. The \hask{if}, \hask{then}, \hask{else} construct defines the elimination rule. 

The fact that, given \hask{h}, we can reconstruct the two terms used to define it, is called the \emph{computation} rule. It tells us how to compute the result of \hask{h}. If the argument evaluates to \hask{True}, then the result is \hask{x}, otherwise it's \hask{y}.
...
data RGB where
  ...
  Blue  :: RGB
\end{haskell}
If you're tired of repeating the same thing over and over, there is a shorthand for this type of definition:
...
\section{Co-Cartesian Categories}
