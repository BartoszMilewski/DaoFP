...
The fact that, given \hask{h}, we can reconstruct the two terms used to define it, is called the \index{computation rule}\emph{computation} rule. It tells us how to compute the result of \hask{h}. If we call \hask{h} with \hask{True}, the result is \hask{x}; if we call it with \hask{False}, the result is \hask{y}.
...
h False = y
\end{haskell}
In fact, there is a third way of writing the same thing using the \index{\hask{case} statement}\hask{case} pattern:
\begin{haskell}
h c = case c of
  Red   -> x
  Green -> y
...
Look at the definition of $1 + 0$ and it's mapping out to any object $a$. It's defined by a pair $(x, \mbox{!`})$, where $x$ is an element of $a$ and $\mbox{!`}$ is the unique arrow from the initial object to $a$ (the \hask{absurd} function in Haskell). 
...
So our $\beta_a$ maps any pair $(x, \mbox{!`})$ to $x$. Conversely, $\beta^{-1}_a$ maps $x$ to the pair $(x, \mbox{!`})$. But is it a natural transformation? 
