...
Look at the definition of $1 + 0$ and it's mapping out to any object $X$. It's defined by a pair $(x, \mbox{!`})$, where $x$ is an element of $X$ and $\mbox{!`}$ is the unique arrow from the initial object to $X$ (the \hask{absurd} function in Haskell). 
