...
This simple idea can be immediately expressed in Haskell as the definition of a type, traditionally called \hask{Bool}, after its inventor George Boole (1815-1864).
...
Let's do a few examples. We haven't defined many types yet, so we'll be limited to mappings out from \hask{Bool} to either \hask{Void}, \hask{()}, or \hask{Bool}. Such edge cases, however, may offer new insights into well known results.
...
\text{Bool}
 \end{tikzcd}
\]
How many pairs of functions from \hask{()} to \hask{Bool} do we have at our disposal? There are only two such functions, \hask{True} and \hask{False}, so we can form four pairs. These are $(True, True)$, $(False, False)$, $(True, False)$, and $(False, True)$. Therefore there can only be four functions from \hask{Bool} to \hask{Bool}. 
