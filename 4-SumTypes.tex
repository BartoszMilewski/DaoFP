...
We can now define a term, or a value, of the type \hask{Bool}, for instance
\begin{haskell}
x :: Bool
x = True
\end{haskell}
...
Our definition of the type \hask{Bool} is still incomplete. We know how to construct a \hask{Bool} term, but we don't know what to do with it, how to ``consume'' it. We have to be able to define arrows that go out of \hask{Bool}---the \index{mapping out}\emph{mappings ou}t of \hask{Bool}. 
...
\subsection{Type theory interpretation}
...
There is a giant enumeration of Unicode characters called \hask{Char}. Their constructors are given special names: you write the character itself between a pair of apostrophes, e.g.,
\begin{haskell}
c :: Char
c = 'a'
\end{haskell}
...
Since the sum is defined by the mapping out property, it was easy to see what happens when we change focus: it changes ``naturally'' with the focus of the arrows that define the coproduct. But what happens when we move the sources of those arrows? 
