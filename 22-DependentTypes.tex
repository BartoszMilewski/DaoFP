...
\end{haskell}
The compiler will recognize this definition as dependently typed if you include the following language pragma:
\begin{haskell}
{-# LANGUAGE DataKinds #-}
\end{haskell}
...
We can be more explicit about the parameters if we import the library:
\begin{haskell}
...
In the \emph{propositions as types} interpretation, type families correspond to families of propositions. The dependent sum type $\Sigma_{x : B} \, T(x)$ corresponds to (the proof relevant version of) the proposition: There exists an $x$ for which $T(x)$ is true:
\[ \exists_{x : B} \, T (x)\]
...
\subsection{Dependent sum in Haskell}

Consider a version of the Haskell \hask{filter} function that operates on vectors. It takes a predicate \hask{a -> Bool} and returns a, possibly shorter, vector of the input elements that satisfy it. The problem is that we don't know up front the size of the output vector. The solution is to return an existential type, something like $\exists_{k : \mathbb N} V(k, a)$. Unfortunately, at this time, Haskell doesn't support existential types. There is a proposal that introduces a new syntax, in which this type would be written as \hask{some (k :: Nat) | Vec k a}. Using this syntax, the signature of the filter function would be:
\begin{haskell}
filterV :: (a -> Bool) -> Vec n a -> some (k :: Nat) | Vec k a
\end{haskell}

Currently the workaround is to use the continuation passing style. Instead of returning an existential, we implement \hask{filterV} to take a handler that is polymorphic in \hask{k}:
\begin{haskell}
...
There is a proposal to add dependent products to Haskell with the new keyword \hask{foreach}. It would allow us to get rid of singletons in our implementation of \hask{replicateV}:
\begin{haskell}
replicateV :: foreach (n :: Nat) -> a -> Vec n a
replicateV Z _  = VNil
replicateV (S n) x = VCons x (replicateV x n)
\end{haskell}
...
This is the principle of substituting equals for equals, which is the basis for \index{equational reasoning}\emph{equational reasoning} in Haskell. We can't encode proofs of equality directly in Haskell, but we can use equational reasoning to reason about Haskell programs. This is one of the main advantages of pure functional programming. You can't perform such substitutions in imperative languages, because of side effects.
