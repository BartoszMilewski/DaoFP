...
We start with the universality of the colimit. If we can construct a cocone from the chain $\Gamma$ to $\text{Colim} (F \Gamma)$ then, by universality, there must be an arrow from $i$ to $\text{Colim} (F \Gamma)$. And the latter, by our assumption, is isomorphic to $F i$. So we'll have a mapping $i \to F i$.
...
\[\text{Colim} (F \Gamma) \cong F i \to i \]
