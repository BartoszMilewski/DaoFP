...
A functor is an arrow between categories. It consumes objects and arrows from one category and produces objects and arrows in another. We can think of it a recipe for building such objects and arrows from the materials provided by the source category. In particular, we  often think of an endofunctor as a container of building materials.
...
Granted, the lifting of arrows by a profunctor can be seen as generalizing composition---just not between profuctors, but rather between hom-sets and profunctors. For instance, we can ``precompose'' $P \langle a, b \rangle$ with an arrow $f \colon s \to a$ to obtain $P \langle s, b \rangle$:
...
To understand this concept better, let's consider a regular functor $F \colon \mathcal{C} \to \mathbf{Set}$ (in other words, a co-presheaf). One way to interpret it is to say that it defines a \index{proof-relevant subset} subset of objects of $\mathcal{C}$, namely those objects that are mapped to non-empty sets. Every element of $F a$ is then treated as a proof that $a$ is a member of this subset. If, on the other hand, $F a$ is empty, then $a$ is not a member of the subset. 

This is in fact a proof-relevant version of defining a subset of a set $S$ using a \index{characteristic function}characteristic function, $\chi \colon S \to \text{Bool}$, which returns $\text{True}$ when an element is in the subset and $\text{False}$ otherwise.
...
\end{haskell}
This is just saying that you can extend the composite profunctor by extending the first one on its left and the second one on its right.
...
\[ Q \langle a, x \rangle \times P \langle x, b \rangle \]
...
\[ Q \langle a, x \rangle \times P \langle y, b \rangle \]
...
\[ P \langle y, b \rangle \times Q \langle a, x \rangle \]
...
between two profunctors of the form:
...
\[ \alpha_x = h \circ i_x \]
This object $c$, together with $i$, is our coend:
\[ c = \int^{x \colon \cat C} P\langle x, x \rangle \]

\begin{exercise}
Verify that, for the extranatural transformation $P \to \Delta_d$, the first extranaturality diamond is equivalent to the cowedge condition, and the second is trivially satisfied.
\end{exercise}
...
In category theory, a functor $F$ is called \index{continuous functor}\emph{continuous} if it preserves limits (and \index{co-continuous functor}co-continuous, if it preserves colimits). It means that, if you have a diagram in the source category, it doesn't matter if you first use $F$ to map the diagram and then take the limit in the target category, or take the limit in the source category and then use $F$ to map this limit. 
...
These versions of the Yoneda lemma, expressed in terms of ends, are often half-jokingly called \index{ninja-Yoneda lemma}ninja-Yoneda lemmas. The fact that the ``integration variable'' is explicit makes them somewhat easier to use in complex formulas.

There is also a dual set of \index{ninja-co-Yoneda lemma}ninja co-Yoneda lemmas that use coends instead. For a covariant functor, we have:
...
Since $S$ was arbitrary, we conclude that the two objects are isomorphic:
\[ \int^{x \colon \mathcal{C}} \mathcal{C}(x, a) \times F x \cong F a \]
...
We've already seen the Yoneda lemma implemented in Haskell. We can now rewrite it in terms of an end. We start by defining a profunctor that will go under the end:
\[ \mathbf{Set} (\mathcal{C}(a, x), F y) \]
Its type constructor takes a functor \hask{f} and a type \hask{a} and generates a profunctor that's contravariant in \hask{x} and covariant in \hask{y}:
\begin{haskell}
...
\end{haskell}
The isomorphism is witnessed by a pair of functions. The first one says that if you have a function \hask{x -> a} and a functorful of \hask{x} then you can make a functorful of \hask{a} simply by using the \hask{fmap}:
\begin{haskell}
...
There is nothing special about coproduct so, in general, a Day convolution is defined in any monoidal category with a tensor product:
...
If we think of functors as containers of values, Day convolution tells us how to combine two different containers into one, when given a function that combines two different values into one. 
...
In Haskell, we can define a type-changing lens as the following \index{existential lens}existential type:
\begin{haskell}
