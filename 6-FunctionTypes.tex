...
When somebody gives you an element of the function object, the only thing you can do with it is to apply it to an element of the argument type using $\varepsilon$. This is how you ``consume'' a function.
...
So far we've been only considering functions of one argument. This is not a real limitation, since we can always implement a function of two arguments as a (single-argument) function that takes a product. The $f$ in the definition of the function object is such a function:
\begin{haskell}
f :: (c, a) -> b
\end{haskell}
...
Another way of looking at the definition of the function object is to interpret the argument type to $h$ as the type of the environment in which $f$ is defined. In that case it's customary to call the environment $\Gamma$. The arrow is interpreted as an expression that uses the variables defined in $\Gamma$. 

Consider a simple example, the expression:
\[a x^2 + b x + c\]
You may think of it as being parameterized by a triple of real numbers $(a, b, c)$ and a variable $x$, taken to be, let's say, a complex number. The triple is an element of a product $\mathbb{R} \times \mathbb{R} \times \mathbb{R}$. We consider this product to be the environment $\Gamma$ for our expression. 
...
The arrow $h \colon \Gamma \to \mathbb{C}^{\mathbb{C}}$ is uniquely determined by the arrow $f$.

In general, the defining diagram for the function object is:
\[
 \begin{tikzcd}
 \Gamma \times a
 \arrow[d, dashed, "{h \times id_a}"']
...
In type theory, the function type constructor takes two arbitrary types, $A$ and $B$ and produces the type $A \to B$.  

The introduction rule is called the \index{lambda abstraction}\emph{lambda abstraction}. We assume that a term $t(x) \colon B$ is defined in an environment in which $x \colon A$. Then $\lambda x . t (x)$ constructs a term of the type $A \to B$.

The elimination rule is function application. Given $x \colon A$ and $f \colon A \to B$, the function application $f x$ yields the term of type $B$. 

The evaluation rule, a.k.a. beta reduction, tells us that the elimination (function application) ``cancels'' the introduction (lambda abstraction): 
\[ \big(\lambda x . t (x)\big) y = t (y)\]
In other words, the only thing you can do with a function is to apply it to an argument. You \emph{evaluate} it -- hence the name of this rule.

More precisely, $t(y)$ is the result of the substitution of $y$ for $x$, often denoted $t [ y/x ]$.
\[ \big(\lambda x . t [x]\big) y = t [y/x] \]

In Haskell, beta reduction can be used in \index{equational reasoning}\emph{equational reasoning}. Symbolically (not actual Haskell), we would write:
\begin{haskell}
(\x -> t[x]) y = t[y/x]
\end{haskell}
where \hask{t[x]} is a term dependent on \hask{x}, and \hask{t[y/x]} is the same term, in which all occurrences of the variable \hask{x} were replaced by the term \hask{y}\footnote{Care has to be taken to make sure that the names of free variables in in \hask{t} and \hask{y} don't collide. This sometimes requires the renaming of parameters, a process called \index{alpha conversion}\emph{alpha conversion}.}. 

Lambda abstractions can be iterated. A lambda of two variables is interpreted as a lambda returning a lambda:
\[ (\lambda x y. \,t)\, x' y' = \Big( \big(\lambda x. (\lambda y . \, t)\big) x' \Big) y' \]

The uniqueness rule, a.k.a. eta conversion, deals with introduction after elimination. Given a function $f \colon A \to B$, abstraction after application cancel each other:
\[ \lambda x . f x = f \] 

\begin{exercise}
Use equational reasoning to show that:
\begin{haskell}
(\ f g x -> f x (g x)) (\a b -> a) (\c d -> c) z
\end{haskell}
reduces to \hask{z}. Or, in math notation:
\[ \big(\lambda f g \,x . f x (g x)\big) ( \lambda a b . a)\, (\lambda c d . c) \, z = z\]
Hint: In the first step, substitute $(\lambda a b . a)$ for $f$.
\end{exercise}
...
Here's the version of the same function taken from the Haskell standard library, where it goes under the name of (lower-case) \hask{either}:
\begin{haskell}
...
Both sum and product are functorial, which means that we can apply functions to their contents. We are ready to translate those diagrams to code. 
...
\end{haskell}
In both cases we call this higher-order function \hask{bimap} since, in Haskell, both the sum and the product are instances of the more general class called \hask{Bifunctor}.

\section{Functoriality of the Function Type}

The function type, or the exponential, is also functorial, but with a twist. We are interested in a mapping from $b^a$ to $d^{c}$, where the objects on the right are related to the ones on the left through some arrows---to be determined. 

The exponential is defined by its mapping-in property, so if we're looking for
\[k \colon b^a \to d^{c} \]
we should draw the diagram that has $k$ as a mapping into $d^{c}$. We get this diagram from the original definition by substituting $b^a$ for $c$:
...
\[g \colon b^a \times c \to d\]
If we find such a $g$, it will uniquely define our $k$.

The way to think about this problem is to consider how we would implement $g$. It takes the product $b^a \times c$ as its argument. Think of it as a pair: an element of the function object $a \to b$ and an element of $c$. The only thing we can do with the function object is to apply it to something. But $b^a$ requires an argument of type $a$, and all we have at our disposal is $c$. We can't do anything unless somebody gives us an arrow $c \to a$. This arrow applied to $c$ will generate the argument for $b^a$. 

However, the result of the application is of type $b$, and $g$ is supposed to produce a $d$. Again, we'll need an arrow $b \to d$ to complete our assignment. 

This may sound complicated, but the bottom line is that we require two arrows. The twist is that the first arrow goes from $c$ to $a$, which feels backward from the usual functoriality considerations. In order to map $b^a$ to $d^{c}$ we need a pair of arrows:
\begin{align*}
f &\colon c \to a \\
g &\colon b \to d 
\end{align*}

This is somewhat easier to explain in Haskell. Our goal is to implement a function \hask{c -> d}, given a function \hask{h :: a -> b}. 

This new function takes an argument of the type \hask{c} so, before we can pass it to \hask{h}, we need to convert \hask{c} to \hask{a}. That's why we need a function \hask{f :: c -> a}. 

Since \hask{h} produces a \hask{b}, and we want to return a \hask{d}, we need another function \hask{g :: b -> d}. All this fits nicely into one higher-order function:
\begin{haskell}
