...
In group theory, \index{Cayley's theorem}Cayley's theorem states that every group is isomorphic to a (subgroup of the) group of permutations. A \index{group}\emph{group} is just a monoid in which every element $g$ has an inverse $g^{-1}$. Permutations are bijective functions that map a set to itself. They \emph{permute} the elements of a set.
...
\end{haskell}
The other direction uses the \index{Yoneda trick}Yoneda trick:
\begin{haskell}
