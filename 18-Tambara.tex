...
Consider a set of all functions $\Set(F *, F *)$ for a given functor $F \colon \cat M \to \Set$. At least some of these are the actions of the monoid, $F m$. 

If we look at the ``neighboring'' set $G *$, for some other functor $G$, and its set of functions  $\Set(G *, G *)$ we'll find there the corresponding actions $G m$. An equivariant function, that is a natural transformation in $[\cat M, \Set]$, will map these actions. It will map other morphisms as well.
...
In group theory, Cayley's theorem states that every group is isomorphic to a (subgroup of the) group of permutations. A \index{group}\emph{group} is just a monoid in which every element has an inverse. Permutations are just functions that map a set to itself.

In category theory, Cayley's theorem is practically built into the definition of a monoid and its representations in $\Set$. The connection between the single-object interpretation and the more traditional set-of-elements interpretation of a monoid is easy to establish. 

We do this by constructing the functor $F \colon \cat M \to \Set$ that maps $*$ to the set $S$ that is given by the hom-set: $S = \cat M(*, *)$. Elements of this set are morphisms in $\cat M$. We define the action of $F$ on morphisms as post-composition:
\[ (F m) n = m \circ n \]
...
At the same time we can look at $S$ as an image of $F \colon \cat M \to \Set$, in which case the functions $S \to S$ form a representation of the monoid. This is the essence of the Cayley's theorem.
...
type DList a = [a] -> [a]
\end{haskell}
To represent a list, we turn it into a function that prepends this list to its argument:
\begin{haskell}
rep :: [a] -> DList a
rep as = \xs -> as ++ xs
\end{haskell}
...
To turn a function back to a list, we act with it on an empty list:
\begin{haskell}
unRep :: DList a -> [a]
unRep f = f []
\end{haskell}
...
\end{haskell}
We can apply this getter to a source value and get a result of type \hask{t}:
\begin{haskell}
...
We end up with a mapping between two natural transformations in the functor category $\cat T$. We can simplify it using the corollary to the Yoneda lemma, resulting in:
...
Recall that the Eilenberg-Moore construction factorizes a monad into a free/forgetful adjunction. This is exactly the adjunction we were looking for when deriving the formula for profunctor optics. What remains is to evaluate the action of $\Phi$ on the representable functor:
...
A type-safe implementation of a traversals would require us to keep track of the sizes of lists. In other words, it would require dependent types.
...
The existential form of a traversal must take into account the fact that the residues for different $n$'s should have, in principle, different types. For instance, you can decompose a tree into an n-tuple of leaves $a^n$ and the residue $c_n$ with $n$ holes. So the correct existential representation for a traversal must involve a coend over all sequences $c_n$ that are indexed by natural numbers:
...
To show the equivalence of the two representations, we first write the mapping out of a sum as a product of mappings:
