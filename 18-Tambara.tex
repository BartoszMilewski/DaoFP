...
The final observation is that the compostion $U \circ F$ of adjoint functors is a monad in the functor category. Let's call this monad $\Phi$. The result is the following identity:
\[ \int_{P \colon \cat T} \Set \big((U P) a, (U P) b\big) \cong \big( \Phi \cat C (a ,-) \big) b \]
The right-hand side is the action of the monad $\Phi$ on the representable functor $\cat C (a, -)$ evaluated at $b$. 
