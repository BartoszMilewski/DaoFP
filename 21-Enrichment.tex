...
Recall that a limit was defined as a representing objects of a functor on the left hand side of the following isomorphism:
\[ [\cat J, \mathcal{C}](\Delta_x, D)  \cong \mathcal{C}(x, \text{Lim}D) \]
where $D \colon \cat J \to \cat C$ was the diagram functor. Let's see if we can generalize this definition to the enriched setting.

Let's assume that both $\cat J$ and $\cat C$ are categories enriched over $\cat V$. This makes $D$ a $\cat V$-enriched functor. The right hand side becomes the hom-object, and the left hand side becomes the object of natural transformations. 

There is just one problem: What is the enriched analog of the constant functor? Its action on objects is simple: all objects in $\cat J$ are mapped to $x$. But how do we define its action on hom-objects? 

An ordinary constant functor $\Delta_x$ maps all morphisms in $\cat J(a, b)$ to the identity morphism $id_x \in \cat C(x, x)$. In the enriched setting, though, $\cat C(x, x)$ is an object with no internal structure. Even if it happened to be the unit $I$, there's no guarantee that for every hom-object $\mathcal J(a, b)$ we could find an arrow to $I$; and even if there was one, it might not be unique. In other words, there is no reason to believe that $I$ is the terminal object in $\cat V$.


Here's another perspective: Limits (and colimits) are built around commuting triangles. Cones are constructed from ``wires,'' that is, individual morphisms. You may think of a hom-set as a bundle of wires, each wire having zero thickness. 

In particular, the wires for the cone with the apex $x$ are selected from the hom-sets $\mathcal C(x, D j)$, where $j$ is an object of $\mathcal J$. 
...
& D l
 \end{tikzcd}
 \]
This selection of a $j$'th wire can be written as a function from the singleton set $1$:
...
where $\Delta_1 \colon \cat J \to \Set$ is a constant functor mapping all objects in $\cat J$ to the singleton set $1$. Naturality conditions ensure that the triangles forming the sides of the cone commute. 

Thus the set of all cones  with the apex $x$ is given by the set of natural transformations:
\[ [\mathcal J, \Set] (\Delta_1, \mathcal C(x, D-)) \]
Earlier we have defined the same set of cones as:
\[ [\cat J, \mathcal{C}](\Delta_x, D) \]
We can now rewrite the definition of a limit using this new representation:
\[ [\mathcal J, \Set] (\Delta_1, \mathcal C(x, D-))  \cong \mathcal{C}(x, \text{Lim}D) \]


This reformulation gets us closer to the enriched setting, since it rephrases the problem in terms of hom-sets rather than individual morphisms. We can re-interpret both $\mathcal J$ and $\mathcal C$ to be enriched over $\mathcal V$, in which case $D$ is a $\mathcal V$-functor. 

We still have a problem with the constant functor. We are looking for an enriched functor to replace $\Delta_1$. It turns out we can use any functor  $W \colon \cat J \to \cat V$ and call it a weight. Instead of selecting a singular wire, like $\Delta_1$ did, this functor ``smears the singularity''---it replaces the wire with a thicker ``cylinder.'' 

For $W$ to be an enriched functor, its target $\cat V$ has to be an enriched category---in this case self-enriched. $\cat V$ must therefore be a monoidal \emph{closed} category.

It can be shown that the mapping that uses $W$ instead of $\Delta_1$:
\[  H_W x = [\mathcal J, \cat V] (W, \mathcal C(x, D-)) \]
is itself a $\cat V$-enriched contravariant functor $H_W \colon \cat C^{op} \to \cat V$. 

A \index{weighted limit}\emph{weighted limit}, also known as an \index{indexed limit}\emph{indexed limit}, $\text{Lim}^W D$, is defined as the representing object for this functor:
\[  [\mathcal J, \cat V] (W, \mathcal C(x, D-)) \cong \cat C (x, \text{Lim}^W D ) \]

Both sides of this natural isomorphism are objects in $\mathcal V$. Tthe left-hand side is the object of natural transformations between two $\mathcal V$-functors, and the right-hand side is the hom-object in $\mathcal C$.

Dually, a weighted colimit is defined by the natural isomorphism:
\[   [\mathcal J^{op}, \mathcal V] (W, \mathcal C(D-, x))  \cong \mathcal C(\text{Colim}^W D, x) \]
...
\begin{exercise}
Show that naturality conditions for the following transformation:
\[ \gamma \colon \Delta_1 \to \mathcal C(x, D -) \]
correspond to commutation condition for the sides of the cone defined by $D$.
\end{exercise}
...
First, let's convince ourselves that this formula works in an ordinary ($\Set$-enriched) category. Since ends are defined by their mapping-in property, let's consider a mapping from an arbitrary object $d$ to the weighted limit and use the standard \index{Yoneda trick}Yoneda trick to show the isomorphism. By definition, we have:
...
Hint: Use the mapping in/out with the \index{Yoneda trick}Yoneda trick and the definition of power and copower.
\end{exercise}
...
Since $d$ was arbitrary, we can use the \index{Yoneda trick}Yoneda trick to conclude that:
