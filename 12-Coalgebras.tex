...
We've seen that, in the category of sets, the initial algebras don't necessarily coincide with terminal coalgebras. The identity functor, for instance, has the empty set as its (carrier of) the initial algebra and the singleton set as its terminal coalgebra. 
...
Data types with a polymorphic constructor are called \emph{existential} types. To construct an element of an existential type, we have the option of picking a particular type, for which we have the data required by the constructor. 
