...
The external hom in any category is always a set. We can therefore generalize the above adjunction to the case where $B$ and $C$ are not sets but objects in some category $\cat C$. Such an adjunction defines the action of a set $A$ on an object $b$:
...
R c' &= d
\end{align*}
$L$ has no chance of undoing it. It can't map $d$ to both $c$ and $c'$. The best it can do is to map $d$ to an object that has arrows to both $c$ and $c'$. These arrows are needed to define the components of the counit of the adjunction:
\begin{align*}
\varepsilon_c &\colon L d \to c
\\
\varepsilon_{c'} &\colon L d \to c'
\end{align*}
since $L d$ is both $L (R c)$ and $L (R c')$
\[
...
The right Kan extension is a functor $\text{Ran}_P F$ equipped with a natural transformation $\varepsilon$, the counit of the Kan extension:
...
The right Kan extension $(\text{Ran}_! D, \varepsilon)$ is the universal such cone. $\text{Ran}_! D$ is a functor from $\mathbf 1$ to $\cat C$, so it selects an object in $\cat C$. This is the apex $c$ of the universal cone. 
...
If we were serious about the interpretation of Kan extensions as franctions, a codensity monad would correspond to $F/F$. A functor for which this is  identity is called codense.

To see that $T^F$ is a monad, we have to define monad multiplication:
...
then the codensity monad is $F \circ L$.
...
\[ \cong \int_d \int_c \Set\big(\cat C(d, F c), \cat D (G d, F c)\big) \]
...
Here's an illustrative example of a simple shape category consisting of three objects and three morphisms (not counting identities). The object $x$ is the image of the single object $*$ under the functor $X$:
\[
 \begin{tikzcd}
1 
\arrow[rr, red]
...
The hint to representing the left Kan extension as a coend comes from the co-Yoneda lemma:
...
We pull out the coend, which becomes an end:
...
Conversely, if the left Kan extension of identity exists and it preserves the functor:
...
Acting on a pair of objects, it's given by the formula:
...
Since we are in $\Set$, the co-power decays to the cartesian product:
\[ \cong \int^{\langle a, b\rangle} \cat C ( a \otimes b, c) \times F a \times G  b \]
which is the formula for Day convolution.

 
\subsection{Kan extensions and optics}

Consider $\cat C^{op} \times \cat D$ an actegory with the action of a monoidal category $\cat M$:
\[ m \bullet \langle c, d \rangle = \langle m \bullet c, m \bullet d \rangle \]
Then the general optic is given by:
\[ \mathcal{O} \langle s, t \rangle \langle a, b \rangle \cong \left(\int^m \text{Lan}_{m \bullet} \;\mathcal Y_{\langle a, b \rangle}\right) \langle s, t \rangle \]
where
...
In fact the profunctor functor:
\[ \Phi P = \int^m \text{Lan}_{m \bullet} \; P \]
is the Pastro-Street monad we used in deriving optics.
