...
If $F$ is a co-presheaf, that is $\cat D$ is $\Set$, we can replace the power with the exponential/hom-set to get:
...
In $\Set$, the co-power reduces to a cartesian product, so the left Kan extension of a co-presheaf becomes:
