...
This works for a co-presheaf. In general we are interested in $F \colon \cat C \to \cat D$, so we need to replace the hom-set in $\Set$ by a \index{power}power. Thus he right Kan extension is given by the following end (if it exists):
...
\end{haskell}
We do it by instantiating this function at the type \hask{c = c'} and calling it with the identity on \hask{(p c')}:
 \begin{haskell}
counit (Ran h) = h id
\end{haskell}
...
For a general functor $F \colon \cat C \to \cat D$, we replace the product with the \index{copower}copower:
...
type Lan p f b = exists c. (p c -> b, f c)
 \end{haskell}
Currently, this is how we would encode the existential using \hask{GADT}'s:
 \begin{haskell}
...
unit :: forall p f c'. 
    f c' -> Lan p f (p c')
\end{haskell}
In other words, given a value of the type \hask{(f c')}, we have to come up with some type \hask{c}, a function \hask{p c -> p c'}, and a value of the type \hask{(f c)}. The obvious choice is to pick \hask{c = c'} and use the identity at \hask{(p c')}:
\begin{haskell}
unit fc = Lan id fc
\end{haskell}

The computational power of left Kan extensions lies in their universal property. Given a functor \hask{g} and a natural transformation from \hask{f} to the composition \hask{g} after \hask{p}:
\begin{haskell}
...
\[ \alpha = (\sigma \circ P) \cdot \eta \]
The whiskering of $\sigma$ means instantiating it at \hask{b = p c}, so the factorization of $\alpha$ is implemented as:
\begin{haskell}
