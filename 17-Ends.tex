...
Notice that in a discrete category, which is just a set of objects with no arrows between them, the co-wedge condition is trivial (there are no $f$s other than identities), so a coend is just a sum (discriminated union) of the diagonal sets $P \langle X, X \rangle$.
...
The existential/universal duality between a \hask{Coend} and an \hask{End} means that it's easy to construct a \hask{Coend}---all you need is to pick one type \hask{x} for which you have a value of the type \hask{p x x}. On the other end, to construct an \hask{End} you have to provide a whole family of values \hask{p x x}, one for every type \hask{x}. In other words, we need a polymorphic formula that is parameterized by \hask{x}. A polymorphic function is a canonical example of such a formula.
...
Notice that the right-hand side is an end.
...
The \hask{get}/\hask{set} version of the lens can be implemented in terms of this existential form.
\begin{haskell}
toGet :: Lens s a -> (s -> a)
...
In Haskell, we can define a lens as an existential type:
\begin{haskell}
...
Since lenses can be composed, you might wonder is there is a category in which lenses describe arrows. Indeed, there is a category $\mathbf{Lens}$ whose objects are pairs of objects in $\mathcal{C}$, and arrows from $\langle S, T\rangle$ to $ \langle A, B \rangle$ are elements of  $\mathit{Lens} \langle S, T\rangle \langle A, B \rangle$.
