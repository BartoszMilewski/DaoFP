...
\subsection{Unit law}
Here's the set/get law or the \emph{unit} law:
\begin{haskell}
set s (get  s) = s
\end{haskell}
...
A type-changing lens generalizes transport to act between bundles. We have to define a whole family of bundles. We start with a category $A$ whose objects define the types that we will use for the foci of our lens. In most cases this is the full category of Haskell types. 

We use a set $B$ as the set of all elements of all types. $B$ is fibrated over $A$---the projection $\pi$ sending an element of $B$ to its corresponding type. You can think of it as the coslice our under-category $1/A$.

The bundle of bundles $E$ is a set fibrated over $B$ with the projection $p$. Since $B$ itself is fibrated over $A$, $E$ is also fibrated over $A$ with the projection $\pi \circ p$. It's this coarser fibration that splits $E$ into a family of bundles. 

We want to treat $E$ as a dependent type that is parameterized by the objects of $A$.
...
The projection $p$ takes an element $s \in E$ and produces an element $b \in B$ whose type is $\pi b$. This is the generalization of \hask{get}. 
