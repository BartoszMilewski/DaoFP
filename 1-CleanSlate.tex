...
x :: A
\end{haskell}
We say that \hask{x} is a term of type \hask{A}, but we'll interpret it as an arrow $x : 1 \to A$, a global element of \hask{A}. \footnote{The Haskell type system distinguishes between \hask{x :: A} and \hask{x :: ()->A}. However, they denote the same thing in categorical semantics.}
